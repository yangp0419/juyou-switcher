...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a juyou-switcher–private field whose SSOT is the DB. Live's
    ...
    match app_type {
        ...
        AppType::Codex => {
            if provider.id == "user" {
                ...
                {
                    log::warn!(
                        "Failed to apply unified Codex session bucket for user restore: {err}"
                    );
                }
                if let Some(config_text) = settings
                    ...
                    .map(str::to_string)
                {
                    match crate::codex_config::preserve_codex_live_global_config_sections(
                        &config_text,
                    ) {
                        Ok(merged_config) => {
                            if let Some(obj) = settings.as_object_mut() {
                                obj.insert(
                                    "config".to_string(),
                                    serde_json::Value::String(merged_config),
                                );
                            }
                        }
                        Err(err) => {
                            log::warn!(
                                "Failed to preserve Codex global config sections for user restore: {err}"
                            );
                        }
                    }
                }
                crate::codex_config::write_codex_live_snapshot(&settings)?;
                return Ok(());
            }
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a juyou-switcher private field that lives only in
            // the DB SSOT plus the `juyou-switcher-model-catalog.json` projection
...
/// into the 聚游助手 database. Each provider found will be added to the
