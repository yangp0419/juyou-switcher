...
///
/// 原因：接管模式下 `*_MODEL` 必须由 聚游助手 写成稳定的 Claude 角色别名，
...
impl ProxyService {
    ...
    /// 强制恢复指定应用的 Live 配置接管。
    ///
    /// 与 `set_takeover_for_app(app, false)` 不同，这里不依赖数据库中的 enabled 标记；
    /// 即使异常退出后只剩 Live 文件里的 `PROXY_MANAGED` 占位符，也会走备份 → SSOT →
    /// 清理占位符的兜底恢复链路。
    pub async fn restore_takeover_for_app(&self, app_type: &str) -> Result<(), String> {
        let app = AppType::from_str(app_type).map_err(|e| format!("无效的应用类型: {e}"))?;
        match app {
            AppType::Claude | AppType::Codex | AppType::Gemini => {}
            _ => return Err("该应用不支持代理接管恢复".to_string()),
        }
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("JUYOU_SWITCH_TEST_HOME").ok();
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "juyou-switcher-model-catalog.json must be created on provider switch"
        );
...
"#;
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the juyou-switcher generated
        ...
        assert!(
            ...
            "restored pointer must still reference the juyou-switcher generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the juyou-switcher catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the juyou-switcher catalog file"
        );
        assert!(
