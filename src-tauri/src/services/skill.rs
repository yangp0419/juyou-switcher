...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.juyou-switcher/skills/`
...
pub enum SkillStorageLocation {
    /// 聚游助手 管理目录 (~/.juyou-switcher/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.juyou-switcher/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.juyou-switcher/skill-backups/）
    ...
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "juyou-switcher".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "juyou-switcher".to_string()));
