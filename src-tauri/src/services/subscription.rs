...
/// - `"codex_oauth"` + "Please re-login via juyou-switcher."（juyou-switcher 自管 OAuth 路径）
...
fn gemini_oauth_client_credentials() -> Option<(String, String)> {
    let client_id = std::env::var("JUYOU_GEMINI_OAUTH_CLIENT_ID")
        .or_else(|_| std::env::var("GEMINI_OAUTH_CLIENT_ID"))
        .ok()?;
    let client_secret = std::env::var("JUYOU_GEMINI_OAUTH_CLIENT_SECRET")
        .or_else(|_| std::env::var("GEMINI_OAUTH_CLIENT_SECRET"))
        .ok()?;
