...
async fn fetch_github_latest_version(client: &reqwest::Client, repo: &str) -> Option<String> {
    ...
    match client
        .get(&url)
        .header("User-Agent", "juyou-switcher")
...
///
/// 判定顺序（命中即返回）：
/// ① Hermes → `<bin_path 绝对> update`;Hermes CLI 自己知道安装环境,避免 juyou-switcher
...
fn launch_macos_terminal(config_file: &std::path::Path, cwd: Option<&Path>) -> Result<(), String> {
    ...
    let temp_dir = std::env::temp_dir();
    let script_file = temp_dir.join(format!("juyou_switcher_launcher_{}.sh", std::process::id()));
...
fn launch_windows_terminal(
    ...
    let bat_file = temp_dir.join(format!("juyou_switcher_claude_{}.bat", std::process::id()));
...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("juyou_switcher_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/usr/bin/env sh
trap 'rm -f "{script_path}"' EXIT
echo "[juyou-switcher] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[juyou-switcher] Command exited. Press Enter to close."
read -r _
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("juyou_switcher_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [juyou-switcher] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [juyou-switcher] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
        );
        std::fs::write(&bat_file, &content).map_err(|e| format!("写入批处理文件失败: {e}"))?;
...
mod tests {
    ...
    mod anchored_upgrade {
        ...
        fn hermes_uses_cli_update_anchor() {
            // Hermes 自带 `hermes update`;锚定到命令行默认那处 CLI,避免 juyou-switcher 猜
    ...
    mod install_strategy {
        ...
        fn hermes_install_uses_official_installer() {
            // Hermes 官方 installer 会处理 Python 3.11+/uv 等运行时;不要再从 juyou-switcher
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("juyou-switcher-missing-{unique}"));
    ...
    fn iterm2_applescript_cold_start_avoids_current_window_before_one_exists() {
        let script = build_macos_iterm2_applescript(Path::new("/tmp/juyou_switcher_launcher.sh"));
    ...
    fn terminal_applescript_cold_start_uses_launch_before_do_script() {
        let script = build_macos_terminal_applescript(Path::new("/tmp/juyou_switcher_launcher.sh"));
        ...
        assert!(
            script
                .contains(r#"set launcher_script to "exec sh '/tmp/juyou_switcher_launcher.sh'""#),
    ...
    fn ghostty_applescript_cold_start_uses_initial_command() {
        let script = build_macos_ghostty_applescript(Path::new("/tmp/juyou_switcher_launcher.sh"));

        // Warm launches execute through the AppleScript command property, not `open -na ... -e`.
        assert!(
            script.contains(r#"set launcher_command to "sh '/tmp/juyou_switcher_launcher.sh'""#),
    ...
    fn dash_c_command_wraps_script_path_inside_quoted_arg() {
        ...
        let s = build_macos_dash_c_command(Path::new("/tmp/juyou_switcher_launcher_1.sh"));
        assert_eq!(s, "exec sh '/tmp/juyou_switcher_launcher_1.sh'");
