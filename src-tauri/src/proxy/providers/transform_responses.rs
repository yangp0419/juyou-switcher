...
pub fn anthropic_to_responses(
    ...
    //   （juyou-switcher 的 transform 当前是"条件写入"，可能产生缺失）
    // - service_tier: 仅在 FAST mode 开启时写入 "priority"
    //   （与 OpenAI 官方 codex-rs 当前请求结构保持一致）
    // - stream: 必须永远 true（codex-rs 硬编码 true，且 juyou-switcher 的
    ...
    if is_codex_oauth {
        ...
        if let Some(obj) = result.as_object_mut() {
            ...
            // 且 juyou-switcher SSE 解析层只支持流式响应。
...
mod tests {
    ...
    fn test_codex_oauth_forces_stream_true_even_when_client_sends_false() {
        ...
        // 依据：juyou-switcher SSE 解析层只支持流式响应
        let input = json!({
            "model": "gpt-5-codex",
            "max_tokens": 1024,
            "stream": false,
