...
/// catalog with a top-level `models` field.  Return the juyou-switcher–managed model
...
/// juyou-switcher–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
    ...
    let active_catalog_path = match crate::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::codex_config::resolve_juyou_switcher_catalog_path(&config_text, &generated_path)
        }
        Err(_) => None,
    };
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to juyou-switcher catalog)"
            );
        }
        json!({"models": []})
    };
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 聚游助手 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a 聚游助手 limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "聚游助手 local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "juyou_switcher_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "juyou_switcher_timeout",
        ProxyError::NoAvailableProvider => "juyou_switcher_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "juyou_switcher_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "juyou_switcher_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "juyou_switcher_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "juyou_switcher_provider_unhealthy",
        ProxyError::ConfigError(_) => "juyou_switcher_config_error",
        ProxyError::TransformError(_) => "juyou_switcher_transform_error",
        ProxyError::InvalidRequest(_) => "juyou_switcher_invalid_request",
        ProxyError::AuthError(_) => "juyou_switcher_auth_error",
        ProxyError::UpstreamError { .. } => "juyou_switcher_upstream_error",
        ProxyError::DatabaseError(_) => "juyou_switcher_database_error",
        ProxyError::Internal(_) => "juyou_switcher_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "juyou_switcher_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("聚游助手 local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "juyou_switcher_forward_failed");
