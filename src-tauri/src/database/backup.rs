...
const JUYOU_SWITCH_SQL_EXPORT_HEADER: &str = "-- 聚游助手 SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_juyou_switcher_sql_export(sql_content)?;
    ...
    fn validate_juyou_switcher_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(JUYOU_SWITCH_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 聚游助手 导出的 SQL 备份文件。",
            "Only SQL backups exported by 聚游助手 are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("juyou-switcher.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("juyou-switcher-periodic-maintenance-backup-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("JUYOU_SWITCH_TEST_HOME", &test_home);
