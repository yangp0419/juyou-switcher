...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("juyouswitcher://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.juyou-switcher/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（按日期输出到 logs/juyou-switcher-YYYY-MM-DD.log）
            {
            ...
            let db_path = app_config_dir.join("juyou-switcher.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 聚游助手 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.juyouswitcher.desktop/applications/juyou-switcher-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/juyou-switcher-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first juyouswitcher:// URL
                        }
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::quick_setup_codex,
            commands::quick_setup_claude,
            ...
            commands::restore_proxy_takeover_for_app,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 juyouswitcher://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older 聚游助手 version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 juyou-switcher.db）\n\
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including juyou-switcher.db)\n\
            2) If you see “database version is newer”, please upgrade 聚游助手\n\
