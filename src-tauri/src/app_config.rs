...
impl SkillApps {
    ...
    pub fn set_enabled_for(&mut self, app: &AppType, enabled: bool) {
        match app {
            ...
            AppType::ClaudeDesktop => {} // Claude Desktop 3P profiles don't use 聚游助手 skill sync
        }
    }
    ...
    /// 其他标签（如 "agents", "juyou-switcher"）忽略。
...
/// 未管理的 Skill（在应用目录中发现但未被 聚游助手 管理）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl MultiAppConfig {
    ...
    pub fn save(&self) -> Result<(), AppError> {
        let config_path = get_app_config_path();
        // 先备份旧版（若存在）到 ~/.juyou-switcher/config.json.bak，再写入新内容
        if config_path.exists() {
