...
/// `providers:` dict. 聚游助手 treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by 聚游助手)
...
/// keeping 聚游助手 free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay 聚游助手 must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a 聚游助手 edit to an unrelated field would silently
...
// 聚游助手 fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    ///
    /// Saves and restores `JUYOU_SWITCH_TEST_HOME` to avoid interfering with
    /// parallel tests in other modules.
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        let _guard = test_guard();
        let tmp = tempfile::tempdir().unwrap();
        let old_test_home = std::env::var_os("JUYOU_SWITCH_TEST_HOME");
        std::env::set_var("JUYOU_SWITCH_TEST_HOME", tmp.path());
        let result = test_fn();
        match old_test_home {
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later 聚游助手 edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a 聚游助手 toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
