...
use crate::codex_config::{
    get_codex_config_dir, read_codex_config_text, JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID,
};
...
const JUYOU_API_HISTORY_UNIFY_MIGRATION_NAME: &str = "codex-juyouapi-history-unify-v1";
...
const LEGACY_JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID: &str = "juyouswitcher";
const LEGACY_JUYOU_API_CODEX_MODEL_PROVIDER_ID: &str = "juyouapi";
...
const JUYOU_SWITCH_LEGACY_CODEX_MODEL_PROVIDER_IDS: &[&str] = &[
    LEGACY_JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID,
    LEGACY_JUYOU_API_CODEX_MODEL_PROVIDER_ID,
    "aicodemirror",
    "aicoding",
    "aigocode",
    "aihubmix",
    ...
    if source_provider_ids.is_empty() {
        crate::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
                ...
                target_provider_id: JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID.to_string(),
...
pub fn migrate_codex_juyouapi_history_to_unified_bucket(
) -> Result<CodexHistoryProviderBucketMigrationOutcome, AppError> {
    let mut source_provider_ids = BTreeSet::new();
    source_provider_ids.insert(LEGACY_JUYOU_API_CODEX_MODEL_PROVIDER_ID.to_string());

    let backup_root = migration_backup_root(JUYOU_API_HISTORY_UNIFY_MIGRATION_NAME);
    let codex_dir = get_codex_config_dir();
    let migrated_jsonl_files =
        migrate_codex_jsonl_files(&codex_dir, &source_provider_ids, &backup_root)?;
    let migrated_state_rows =
...
/// 迁移前 jsonl / state DB 均备份到 `~/.juyou-switcher/backups/codex-official-history-unify-v1/`。
    ...
    config_text
        ...
        .and_then(|doc| {
            doc.get("model_provider")
                ...
                .map(|id| id.trim() == JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID)
        })
        .unwrap_or(false)
}
...
fn rewrite_codex_session_meta_line_for_restore(
    ...
    if payload.get("model_provider")?.as_str()? != JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID {
        return None;
    }
...
fn restore_codex_state_db_official_threads(
    ...
    for chunk in ids.chunks(STATE_DB_ID_CHUNK) {
        ...
        values.push(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID.to_string());
...
fn collect_source_model_provider_ids(db: &Database) -> Result<BTreeSet<String>, AppError> {
    ...
    for provider in providers.values() {
        ...
        insert_known_juyou_switcher_legacy_source_id(&mut ids, &provider.id);
        ...
        for provider_id in trusted_legacy_codex_model_provider_ids_from_config(config_text) {
            insert_known_juyou_switcher_legacy_source_id(&mut ids, &provider_id);
        }
...
fn insert_known_juyou_switcher_legacy_source_id(ids: &mut BTreeSet<String>, provider_id: &str) {
    let trimmed = provider_id.trim();
    if is_known_juyou_switcher_legacy_codex_model_provider_id(trimmed) {
...
fn is_known_juyou_switcher_legacy_codex_model_provider_id(provider_id: &str) -> bool {
    JUYOU_SWITCH_LEGACY_CODEX_MODEL_PROVIDER_IDS
        .iter()
...
fn legacy_codex_model_provider_id_from_normalized_config(config_text: &str) -> Option<String> {
    ...
    let provider_id = doc
        ...
        .and_then(|item| item.as_str())
        .map(str::trim)?;
    if provider_id != JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID
        && provider_id != LEGACY_JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID
    {
        return None;
    }
...
fn normalized_legacy_codex_provider_name(name: &str) -> Option<&'static str> {
    if is_known_juyou_switcher_legacy_codex_model_provider_id(name) {
        return JUYOU_SWITCH_LEGACY_CODEX_MODEL_PROVIDER_IDS
            .iter()
            .copied()
...
fn insert_trusted_legacy_config_model_provider_id(
    ...
    if provider_id.is_empty()
        || !is_known_juyou_switcher_legacy_codex_model_provider_id(provider_id)
...
fn migrate_provider_config_template_to_custom(
    ...
    let custom_table_exists =
        config_defines_model_provider(&doc, JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(source_provider_id) = source_provider_id_to_move {
        ...
        model_providers[JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID] = provider_table;
        changed = true;
    }
    ...
    for profile_key in profile_keys {
        ...
        if references_legacy {
            profile_table.insert(
                "model_provider",
                toml_edit::value(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID),
            );
            changed = true;
        }
    }
...
fn rewrite_codex_session_meta_line(
    ...
    payload.insert(
        "model_provider".to_string(),
        Value::String(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID.to_string()),
    );
    serde_json::to_string(&value).ok()
}
...
"#
            ...
            Provider::with_id(
                "legacy-juyouswitcher".to_string(),
                "Legacy 聚游助手".to_string(),
                serde_json::json!({
                    "auth": {},
                    "config": r#"model_provider = "juyouswitcher"

[model_providers.juyouswitcher]
name = "AIHubMix"
base_url = "https://aihubmix.example/v1"
"#
                }),
...
"#
        ...
        assert_eq!(
            source_provider_ids,
            source_ids(&["aihubmix", "juyouswitcher", "rightcode"])
        );
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"juyouswitcher\"}}\n",
        ...
        conn.execute_batch(
            ...
            INSERT INTO threads (id, model_provider) VALUES
                ...
                ('juyouswitcher-thread', 'juyouswitcher'),
        ...
        assert_eq!(
            ...
            source_ids(&["legacy-juyouswitcher", "rightcode"])
        );
        ...
        let juyouswitcher_config: toml::Value =
            toml::from_str(&config_provider_id("legacy-juyouswitcher"))
                .expect("parse juyouswitcher config");
        assert_eq!(
            juyouswitcher_config
                .get("model_provider")
                .and_then(|value| value.as_str()),
            Some("custom")
        );
        assert!(juyouswitcher_config
            .get("model_providers")
            .and_then(|value| value.get("juyouswitcher"))
            .is_none());
...
"#
    ...
    #[test]
    fn collects_legacy_juyouswitcher_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            ...
            serde_json::json!({
                "auth": {},
                "config": "model_provider = \"juyouswitcher\"\n\n[model_providers.juyouswitcher]\nname = \"AIHubMix\"\nbase_url = \"https://aihubmix.example/v1\""
            }),
            None,
        );
        provider.category = Some("aggregator".to_string());
        ...
        let ids = collect_source_model_provider_ids(&db).expect("collect ids");
        assert!(ids.contains("juyouswitcher"));
        assert!(ids.contains("aihubmix"));
        assert!(!ids.contains("generated-uuid"));
    }

    #[test]
    fn collects_legacy_juyouapi_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "juyouapi".to_string(),
            "Juyou API".to_string(),
...
"#
    ...
    #[test]
    fn migrates_legacy_juyouswitcher_provider_template_to_custom() {
        let db = Database::memory().expect("memory db");
        let provider = Provider::with_id(
            "legacy-juyouswitcher".to_string(),
            "Legacy 聚游助手".to_string(),
...
"#
        ...
        assert_eq!(
            ...
            vec!["legacy-juyouswitcher".to_string()]
        );

        let saved = db
            .get_provider_by_id("legacy-juyouswitcher", "codex")
            .expect("get provider")
            .expect("provider exists");
        let config_text = saved
            .settings_config
...
"#
    ...
    #[test]
    fn skips_custom_category_unknown_provider_when_created_by_juyou_switcher() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            "Manual Relay".to_string(),
