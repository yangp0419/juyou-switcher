...
pub const JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID: &str = "custom";
pub const JUYOU_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "juyou-switcher-model-catalog.json";
const CODEX_MODEL_CATALOG_TEMPLATE_SLUG: &str = "gpt-5.5";
const JUYOU_API_CODEX_PROVIDER_NAME: &str = "Juyou API";
const JUYOU_API_CODEX_BASE_URL: &str = "https://api.juyouhuyu.top/v1";
const CODEX_APP_BUNDLE_PATH: &str = "/Applications/Codex.app";
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(JUYOU_SWITCH_CODEX_MODEL_CATALOG_FILENAME)
}
...
const CODEX_GLOBAL_CONFIG_KEYS: &[&str] = &[
    "notify",
    "desktop",
    "projects",
    "marketplaces",
...
fn merge_codex_global_config_sections(
    target_config: &str,
    live_config: &str,
...
pub fn preserve_codex_live_global_config_sections(config_text: &str) -> Result<String, AppError> {
    let live_config = read_codex_config_text().unwrap_or_default();
    merge_codex_global_config_sections(config_text, &live_config)
}

/// Restore a Codex live snapshot exactly as captured by `read_codex_live_settings`.
///
/// This is intentionally different from provider switching:
/// - provider switching may move API keys into provider-scoped TOML fields and
///   apply model catalog projections;
/// - snapshot restore must write the stored `auth` and raw `config.toml` text
///   back as-is so user-owned Codex settings such as `notify`, `desktop`,
///   `projects`, plugins, and MCP servers round-trip without being reinterpreted.
pub fn write_codex_live_snapshot(settings: &Value) -> Result<(), AppError> {
    ...
    match (auth, config_text) {
        ...
        (Some(auth), None) => {
            if auth.as_object().is_some_and(|obj| obj.is_empty()) {
                let _ = delete_file(&get_codex_auth_path());
                Ok(())
            } else {
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(_) => {
            doc["model_catalog_json"] = toml_edit::value(JUYOU_SWITCH_CODEX_MODEL_CATALOG_FILENAME);
        }
        None => {
            let should_remove = doc
                ...
                .map(|path| {
                    Path::new(path).file_name().and_then(|name| name.to_str())
                        == Some(JUYOU_SWITCH_CODEX_MODEL_CATALOG_FILENAME)
                })
                .unwrap_or(false);
            if should_remove {
...
/// juyou-switcher–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// juyou-switcher–generated file (identified by filename
/// `juyou-switcher-model-catalog.json`). A user-managed external catalog file is
...
pub fn read_codex_model_catalog_simplified_from_live() -> Result<Option<Value>, AppError> {
    ...
    let Some(catalog_path) = resolve_juyou_switcher_catalog_path(&config_text, &generated_path)
    else {
        return Ok(None);
    };
...
/// Given `config.toml` text, resolve the on-disk path of the juyou-switcher–owned
...
pub(crate) fn resolve_juyou_switcher_catalog_path(
    config_text: &str,
    generated_path: &Path,
) -> Option<PathBuf> {
    ...
    let is_juyou_switcher_owned = referenced_path.file_name().and_then(|name| name.to_str())
        == Some(JUYOU_SWITCH_CODEX_MODEL_CATALOG_FILENAME);
    if !is_juyou_switcher_owned {
        return None;
    }
...
pub fn read_codex_live_settings() -> Result<Value, AppError> {
    ...
    let cfg_text = match read_and_validate_codex_config_text() {
        Ok(text) if !text.trim().is_empty() => text,
        _ => {
            // 如果配置文件不存在或为空，使用默认的 Juyou API 配置
            if !auth_present {
                log::info!("Codex 配置文件不存在，使用默认 Juyou API 配置");
                get_default_juyou_codex_config()
            } else {
                // 有 auth 但没有 config，返回空配置
                String::new()
            }
        }
    };
...
/// 返回默认的 Juyou API 配置
fn get_default_juyou_codex_config() -> String {
    r#"model = "gpt-5.5"
...
fn ensure_codex_default_plugin_config(doc: &mut DocumentMut) {
    let codex_dir = get_codex_config_dir();

    if doc.get("notify").is_none() {
        let computer_use_client = codex_dir
            .join("computer-use")
            .join("Codex Computer Use.app")
            .join("Contents/SharedSupport/SkyComputerUseClient.app/Contents/MacOS/SkyComputerUseClient");
        if computer_use_client.exists() {
            let mut notify = toml_edit::Array::new();
            notify.push(computer_use_client.to_string_lossy().to_string());
            notify.push("turn-ended");
            doc["notify"] = toml_edit::value(notify);
        }
    }

    ensure_table(doc, "marketplaces");
    if let Some(marketplaces) = doc["marketplaces"].as_table_mut() {
        if !marketplaces.contains_key("openai-bundled") {
            let mut bundled = toml_edit::Table::new();
            bundled["source_type"] = toml_edit::value("local");
            bundled["source"] = toml_edit::value(
                codex_dir
                    .join(".tmp/bundled-marketplaces/openai-bundled")
                    .to_string_lossy()
                    .to_string(),
            );
            marketplaces["openai-bundled"] = toml_edit::Item::Table(bundled);
        }
    }
    ...
    if let Some(plugins) = doc["plugins"].as_table_mut() {
        for plugin_id in [
            ...
            if !plugins.contains_key(plugin_id) {
                ...
                plugins[plugin_id] = toml_edit::Item::Table(plugin);
            }
        }
    }

    let codex_app = Path::new(CODEX_APP_BUNDLE_PATH);
    let node_repl_command = codex_app.join("Contents/Resources/cua_node/bin/node_repl");
    if !node_repl_command.exists() {
        return;
    }

    ensure_table(doc, "mcp_servers");
    let Some(mcp_servers) = doc["mcp_servers"].as_table_mut() else {
        return;
    };
    if mcp_servers.contains_key("node_repl") {
        return;
    }

    let mut node_repl = toml_edit::Table::new();
    node_repl["args"] = toml_edit::value(toml_edit::Array::new());
    node_repl["command"] = toml_edit::value(node_repl_command.to_string_lossy().to_string());
    node_repl["startup_timeout_sec"] = toml_edit::value(120);

    let mut env = toml_edit::Table::new();
    env["NODE_REPL_NATIVE_PIPE_CONNECT_TIMEOUT_MS"] = toml_edit::value("1000");
    env["NODE_REPL_NODE_MODULE_DIRS"] = toml_edit::value(
        codex_app
            .join("Contents/Resources/cua_node/lib/node_modules")
            .to_string_lossy()
            .to_string(),
    );
    env["NODE_REPL_NODE_PATH"] = toml_edit::value(
        codex_app
            .join("Contents/Resources/cua_node/bin/node")
            .to_string_lossy()
            .to_string(),
    );
    env["NODE_REPL_TRUSTED_CODE_PATHS"] = toml_edit::value(codex_dir.to_string_lossy().to_string());
    env["CODEX_HOME"] = toml_edit::value(codex_dir.to_string_lossy().to_string());
    env["BROWSER_USE_AVAILABLE_BACKENDS"] = toml_edit::value("chrome,iab");
    env["NODE_REPL_INSTRUCTIONS_USE_CASE_BROWSER"] =
        toml_edit::value("Control the in-app browser in conjunction with the Browser Plugin.");
    env["NODE_REPL_INSTRUCTIONS_USE_CASE_CHROME"] = toml_edit::value("Control the Chrome browser in conjunction with the Chrome Plugin. Prefer this method of controlling Chrome over alternatives (such as Computer Use) unless the user explicitly mentions an alternative.");
    env["BROWSER_USE_CODEX_APP_BUILD_FLAVOR"] = toml_edit::value("prod");
    env["CODEX_CLI_PATH"] = toml_edit::value(
        codex_app
            .join("Contents/Resources/codex")
            .to_string_lossy()
            .to_string(),
    );
    node_repl["env"] = toml_edit::Item::Table(env);
    mcp_servers["node_repl"] = toml_edit::Item::Table(node_repl);
}
...
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    let active_provider = doc
        ...
        .map(str::to_string);
    if active_provider.as_deref().is_some_and(|provider_id| {
        provider_id != "openai" && provider_id != JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID
    }) {
        return Ok(config_text.to_string());
    }
    ...
    let existing_custom_conflicts = doc
        .get("model_providers")
        .and_then(|item| item.as_table())
        .and_then(|providers| providers.get(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID))
        .and_then(|item| item.as_table())
    ...
    doc["model_provider"] = toml_edit::value(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(providers) = doc["model_providers"].as_table_mut() {
        if !providers.contains_key(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID) {
            providers.insert(
                JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID,
...
pub fn strip_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if doc.get("model_provider").and_then(|item| item.as_str())
        != Some(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID)
    {
        return Ok(config_text.to_string());
    }
    ...
    let providers_empty = doc["model_providers"]
        .as_table_mut()
        .map(|providers| {
            providers.remove(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID);
            providers.is_empty()
        })
        .unwrap_or(false);
    if providers_empty {
...
pub fn write_codex_live_for_provider(
    ...
    let config_with_global_sections = config_text
        .map(preserve_codex_live_global_config_sections)
        .transpose()?;
    let config_text = config_with_global_sections.as_deref().or(config_text);
...
mod tests {
    ...
    impl TestHome {
        fn new() -> Self {
            ...
            let original_test_home = std::env::var_os("JUYOU_SWITCH_TEST_HOME");
            std::env::set_var("JUYOU_SWITCH_TEST_HOME", dir.path());
            Self {
                _dir: dir,
                original_test_home,
            }
    ...
    impl Drop for TestHome {
        fn drop(&mut self) {
            match &self.original_test_home {
                Some(value) => std::env::set_var("JUYOU_SWITCH_TEST_HOME", value),
                None => std::env::remove_var("JUYOU_SWITCH_TEST_HOME"),
            }
        }
    }
    ...
    fn unified_session_bucket_injects_for_empty_official_config() {
        ...
        let custom = doc["model_providers"][JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID]
            .as_table()
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"juyou-switcher-model-catalog.json\"\n";
        ...
        let explicit_openai = "model_provider = \"openai\"\n";
        let injected_openai =
            inject_codex_unified_session_bucket(explicit_openai).expect("inject openai");
        assert!(injected_openai.contains("model_provider = \"custom\""));
        assert!(injected_openai.contains("requires_openai_auth = true"));

        // 用户显式指定其它 model_provider 的官方配置不被覆盖
...
"#;
    ...
    #[test]
    #[serial]
    fn write_juyou_api_config_routes_history_to_custom_bucket() {
        let _home = TestHome::new();
        let existing_config = r#"notify = ["turn-ended"]
...
trust_level = "trusted"
"#;
        write_codex_live_atomic(&json!({}), Some(existing_config)).expect("seed live config");

        write_juyou_api_config_to_codex("new-token", "gpt-5.5").expect("write juyou config");

        let config_text = fs::read_to_string(get_codex_config_path()).expect("read config");
        ...
        assert_eq!(
            ...
            Some(JUYOU_API_CODEX_BASE_URL)
        );
        assert!(
            parsed
                .get("model_providers")
                .and_then(|providers| providers.get("juyouapi"))
                .is_none(),
            "legacy juyouapi table should be removed"
        );
        assert_eq!(
            parsed
                .get("projects")
                .and_then(|projects| projects.get("/tmp/project"))
                .and_then(|project| project.get("trust_level"))
                .and_then(toml::Value::as_str),
            Some("trusted"),
            "unrelated Codex config should be preserved"
        );
        assert!(
            parsed
                .get("marketplaces")
                .and_then(|marketplaces| marketplaces.get("openai-bundled"))
                .is_some(),
            "one-click setup should seed bundled marketplace config"
        );
        for plugin_id in [
            "browser@openai-bundled",
            "computer-use@openai-bundled",
            ...
            assert_eq!(
                ...
                Some(true),
                "one-click setup should enable {plugin_id}"
            );
        }
    }
    ...
    #[test]
    #[serial]
    fn write_live_snapshot_restores_raw_user_config_and_removes_empty_auth() {
        let _home = TestHome::new();
        let juyou_config = r#"model = "gpt-5.5"
model_provider = "juyouapi"

[model_providers.juyouapi]
name = "Juyou API"
base_url = "https://api.juyouhuyu.top/v1"
experimental_bearer_token = "sk-juyou"
wire_api = "responses"
"#;
        write_codex_live_atomic(&json!({"OPENAI_API_KEY": "sk-juyou"}), Some(juyou_config))
            .expect("seed juyou live config");

        let user_config = r#"notify = ["/Users/yangp/.codex/computer-use/Codex Computer Use.app/Contents/SharedSupport/SkyComputerUseClient.app/Contents/MacOS/SkyComputerUseClient", "turn-ended"]

[desktop]
followUpQueueMode = "queue"

[projects."/Users/yangp/coding/juyou-switch"]
trust_level = "trusted"
...
"#;
        write_codex_live_snapshot(&json!({
            "auth": {},
            "config": user_config,
        }))
        .expect("restore user snapshot");

        assert_eq!(
            std::fs::read_to_string(get_codex_config_path()).expect("read restored config"),
            user_config
        );
        assert!(
            !get_codex_auth_path().exists(),
            "empty snapshot auth should remove auth.json"
        );
    }
...
"#;
    ...
    #[test]
    fn codex_global_config_sections_survive_provider_switch() {
...
wire_api = "responses"
"#;
        let live = r#"model_provider = "openai"
notify = ["/Applications/Codex.app/notify", "turn-ended"]
...
"#;
        ...
        let merged = merge_codex_global_config_sections(target, live).expect("merge");
        ...
        assert!(
            parsed
                ...
                .is_none(),
            "provider routing sections should not be copied from live config"
        );
        assert_eq!(
            parsed
                .get("plugins")
        ...
        assert!(
            parsed.get("marketplaces").is_some(),
            "marketplace config should be preserved"
        );
        assert!(
        ...
        assert!(
            ...
            "project trust config should be preserved"
        );
    }
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/juyou-switcher-model-catalog.json");
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/juyou-switcher-model-catalog.json");
        assert!(resolve_juyou_switcher_catalog_path("", &generated).is_none());
        assert!(
            resolve_juyou_switcher_catalog_path("model = \"gpt-5\"", &generated).is_none(),
            "no model_catalog_json field should yield None"
        );
    }

    #[test]
    fn resolve_catalog_path_accepts_juyou_switcher_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/juyou-switcher-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/juyou-switcher-model-catalog.json"
"#;
        let resolved =
            resolve_juyou_switcher_catalog_path(config, &generated).expect("path resolves");
...
"#;
        assert!(
            resolve_juyou_switcher_catalog_path(config, &generated).is_none(),
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as juyou-switcher would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\juyou-switcher-model-catalog.json");
        ...
        assert_eq!(
            written_path, JUYOU_SWITCH_CODEX_MODEL_CATALOG_FILENAME,
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/juyou-switcher-model-catalog.json");
    ...
    #[test]
    fn set_catalog_json_none_removes_juyou_switcher_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/juyou-switcher-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove juyou-switcher-owned field regardless of path format"
        );
    }
...
model_catalog_json = "juyou-switcher-model-catalog.json"
"#;
        let generated_path = PathBuf::from("/home/user/.codex/juyou-switcher-model-catalog.json");
        let result = resolve_juyou_switcher_catalog_path(config_text, &generated_path);
        assert_eq!(
            result,
            Some(generated_path),
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by juyou-switcher"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "juyou-switcher-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative juyou-switcher-owned field"
        );
    }
}

/// 直接写入 Juyou API 配置到 config.toml（用于一键配置）
///
/// 此函数会：
/// 1. 读取现有的 config.toml（如果存在）
/// 2. 保留现有的其他配置（如项目信任级别）
/// 3. 添加/更新 Juyou API 的模型配置
/// 4. 写入 experimental_bearer_token
pub fn write_juyou_api_config_to_codex(api_key: &str, model_name: &str) -> Result<(), AppError> {
    let config_path = get_codex_config_path();

    // 读取现有配置（如果存在）
    let existing_content = if config_path.exists() {
        fs::read_to_string(&config_path)
            .map_err(|e| AppError::Config(format!("读取 config.toml 失败: {}", e)))?
    } else {
        String::new()
    };
    ...
    // 设置模型配置。Codex 会按 model_provider 给历史分桶；这里使用项目统一的
    // custom 桶，让 Juyou 和其它第三方配置共享会话历史。
    doc["model"] = toml_edit::value(model_name);
    doc["model_provider"] = toml_edit::value(JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID);

    // 创建 model_providers.custom 表
    let mut juyou_table = toml_edit::Table::new();
    juyou_table["name"] = toml_edit::value(JUYOU_API_CODEX_PROVIDER_NAME);
    juyou_table["base_url"] = toml_edit::value(JUYOU_API_CODEX_BASE_URL);
    juyou_table["experimental_bearer_token"] = toml_edit::value(&bearer_token);
    juyou_table["wire_api"] = toml_edit::value("responses");
    ...
    // 设置统一 custom provider
    if let Some(providers) = doc["model_providers"].as_table_mut() {
        providers[JUYOU_SWITCH_CODEX_MODEL_PROVIDER_ID] = toml_edit::Item::Table(juyou_table);
        providers.remove("juyouapi");
    }

    // 设置 features
    let mut features_table = toml_edit::Table::new();
    features_table["js_repl"] = toml_edit::value(false);
    doc["features"] = toml_edit::Item::Table(features_table);
    ensure_codex_default_plugin_config(&mut doc);

    // 写入文件
    let content = doc.to_string();
    atomic_write(&config_path, content.as_bytes())
        .map_err(|e| AppError::Config(format!("写入 config.toml 失败: {}", e)))?;

    log::info!("成功写入 Juyou API 配置到 {:?}", config_path);
    Ok(())
}
