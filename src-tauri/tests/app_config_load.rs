...
use juyou_switcher_lib::{AppError, MultiAppConfig};
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(".juyou-switcher").join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&juyou_switcher_lib::AppType::Claude)
        .is_some());
    assert!(loaded
        .get_manager(&juyou_switcher_lib::AppType::Codex)
        .is_some());
}
