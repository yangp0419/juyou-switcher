...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".juyou-switcher").join("settings.json")
}
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = juyou_switcher_lib::get_claude_settings_path();
    ...
    let db_path = std::path::Path::new(&home_dir)
        .join(".juyou-switcher")
        .join("juyou-switcher.db");
    assert!(
        db_path.exists(),
        "switching provider should persist to juyou-switcher.db"
    );
