...
```
~/.juyou-switcher/
├── juyou-switcher.db      # SQLite データベース（SSOT）
...
`juyou-switcher.db` は SQLite データベースで、以下を保存しています：
...
Hermes は YAML 設定を使用します。聚游助手 は MCP サーバーを `mcp_servers` に書き込み、編集可能なプロバイダーエントリを `custom_providers` に書き込み、Hermes の `providers` dict にある読み取り専用エントリを読み取り、プロバイダー切り替え時に `model.provider` / `model.default` を更新します。
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（聚游助手 の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
- CLI ツールの設定ファイル（聚游助手 がバックフィルする）
- 聚游助手 の `settings.json`

### 手動編集を推奨しないもの

- `juyou-switcher.db` データベースファイル
- バックアップファイル
...
聚游助手 v3.7.0 で JSON ファイルから SQLite に移行しました：
