...
```
~/.juyou-switcher/
├── juyou-switcher.db      # SQLite 数据库（SSOT）
├── settings.json     # 设备级设置
├── skills/           # 技能 SSOT 目录
...
`juyou-switcher.db` 是 SQLite 数据库，存储：
...
Hermes 使用 YAML 配置。聚游助手 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 聚游助手 的"供应商"） |
| `env` | 环境变量配置 |
...
- CLI 工具的配置文件（会被 聚游助手 回填）
- 聚游助手 的 `settings.json`

### 不建议手动编辑

- `juyou-switcher.db` 数据库文件
- 备份文件
...
聚游助手 v3.7.0 从 JSON 文件迁移到 SQLite：
