...
聚游助手 automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with 聚游助手 configuration
[Expand] [Dismiss]
```
...
3. 聚游助手 will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.juyou-switcher/env-backups/`
...
If you prefer not to use 聚游助手 to remove variables, you can handle them manually:
...
- 聚游助手 provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use 聚游助手 to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.juyou-switcher/env-backups/`
