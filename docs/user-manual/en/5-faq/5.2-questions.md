...
聚游助手 for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/yangp0419/juyou-switcher/releases).
...
2. Add 聚游助手 to your antivirus software's whitelist
...
chmod +x juyou-switcher-*.AppImage

# If it still fails, try
./juyou-switcher-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.juyou-switcher/` directory exists
2. Restore from backup: `~/.juyou-switcher/backups/`
...
**Solutions**:
1. Confirm the file was exported by 聚游助手
...
**Windows**:
- Check taskbar settings to ensure the 聚游助手 icon is not hidden
...
2. Restart the app
3. Delete `~/.juyou-switcher/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask juyou-switcher`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and 聚游助手 runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, 聚游助手 covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `juyouswitcher://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/yangp0419/juyou-switcher/issues)
...
- macOS/Linux: `~/.juyou-switcher/logs/`
- Windows: `%APPDATA%\juyou-switcher\logs\`
