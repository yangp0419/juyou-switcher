...
The Claude Desktop panel lets you manage Claude Desktop provider configurations in 聚游助手.
...
Claude Desktop and Claude Code are separate app entry points. Claude Code uses `~/.claude/settings.json`, while Claude Desktop uses its own 3P profile configuration. In 聚游助手 they appear as separate apps: "Claude" and "Claude Desktop"; the icon badge in the bottom-right corner helps distinguish them.
...
| Third-party mode | Writes the 3P profile managed by 聚游助手 |
| MCP / Skills | Claude Desktop 3P profiles do not use 聚游助手 MCP / Skills sync |
...
- Providers that need routing: keep 聚游助手 running, turn on Claude Desktop local routing, then restart Claude Desktop
...
In direct mode, 聚游助手 points the Claude Desktop 3P profile to the provider endpoint:
...
- 聚游助手 local routing does not need to stay running while in use
...
Model mapping mode is for providers whose models are not `claude-sonnet-*` / `claude-opus-*` / `claude-haiku-*` role IDs (including legacy Claude IDs and non-Claude models such as DeepSeek and Kimi), or when 聚游助手 needs to convert the API format.

After **Needs model mapping** is enabled, Claude Desktop connects to the 聚游助手 local gateway:
...
4. Using the provider credentials saved in 聚游助手 to call upstream
...
The reason is that Claude Desktop now rejects models outside the Sonnet / Opus / Haiku role families, so 聚游助手 local routing performs a model mapping pass.
...
Model mapping mode needs 聚游助手 local routing to convert requests. Local routing is powerful but more complex, so the main-page routing toggle is hidden by default to avoid accidental clicks by users who do not need routing. When you need routing, show it manually.
...
聚游助手 restores Claude Desktop's official 1P mode and removes the 3P profile managed by 聚游助手.

Official mode does not need an API key or local routing.

When importing from Claude Code, 聚游助手 automatically adds **Claude Desktop Official**.

## Configuration File Locations

聚游助手 writes Claude Desktop 3P configuration files.
...
These files are maintained automatically by 聚游助手. Manual edits are not recommended. If configuration becomes inconsistent, enabling the current provider again usually repairs it.
...
| Local routing token has not been generated | Switch to the provider again; 聚游助手 will write a new local token |
...
- Model mapping mode: 聚游助手 must keep running, and Claude Desktop local routing must stay on

### Are real upstream model names written into Claude Desktop?

Not in model mapping mode. The Claude Desktop profile only stores safe Sonnet / Opus / Haiku role routes and display names. Real upstream model names stay in the 聚游助手 provider configuration and are mapped when requests pass through the local gateway.
