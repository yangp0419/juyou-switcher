...
聚游助手 comes pre-configured with the following GitHub repositories:
...
聚游助手 provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.juyou-switcher/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode, Hermes)
- Removes the skill from the SSOT directory (`~/.juyou-switcher/skills/`)
...
聚游助手 compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. 聚游助手 batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **聚游助手 built-in**   | Default location `~/.juyou-switcher/skills/`, managed by 聚游助手         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — 聚游助手 smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside 聚游助手.
...
- 聚游助手 has not finished the latest scan
