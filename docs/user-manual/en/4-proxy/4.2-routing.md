...
App routing means letting 聚游助手 route a specific application's API requests through the local routing service.
...
When routing is enabled, 聚游助手 modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, 聚游助手 will:
...
Before enabling routing, 聚游助手 backs up the original configuration and restores it when disabled.
