...
This section describes how to configure 聚游助手 according to your preferences.
...
When enabled, 聚游助手 automatically runs when the system starts.
...
Starting from v3.13.0, 聚游助手 adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger 聚游助手 via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If 聚游助手 runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, 聚游助手 automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that 聚游助手 uses when opening a terminal.
...
The storage location for 聚游助手's own data, defaulting to `~/.juyou-switcher/`.
...
The exported file name format is `juyou-switcher-export-{timestamp}.sql`.
...
Configure 聚游助手's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, 聚游助手 automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `juyou-switcher-sync`) |
...
- 聚游助手 automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, 聚游助手 displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current 聚游助手 version number, with support for:
