...
> ## 唯一の公式チャネル（必ずお読みください）
>
> 聚游助手 は**完全に無料・オープンソース**のデスクトップアプリで、**ユーザーから料金を徴収することはありません**。本ソフトウェアは下記の公式チャネルからのみ入手してください：
>
> | チャネル     | 唯一の公式                                                                     |
> | ------------ | ------------------------------------------------------------------------------ |
> | 公式サイト   | **[github.com/yangp0419/juyou-switcher](https://github.com/yangp0419/juyou-switcher)**                                         |
> | ソースコード | **[github.com/yangp0419/juyou-switcher](https://github.com/yangp0419/juyou-switcher)** |
> | ダウンロード | **[GitHub Releases](https://github.com/yangp0419/juyou-switcher/releases)**        |
> | 作者         | **[@yangp0419](https://github.com/yangp0419)**                               |
> | 偽サイト通報 | **[GitHub Issues](https://github.com/yangp0419/juyou-switcher/issues)**            |
>
> **料金請求・チャージ・認証情報の提供を求める「聚游助手」サイトやクライアントはすべて偽物です。** 支払いを誘導された場合は直ちに操作を中止し、GitHub Issues からご報告ください。

---

## 概要

聚游助手 v3.16.2 は v3.16.1 に続くメンテナンスアップデートです。前リリースでは Codex 公式認証とローカルルーティングのテイクオーバーのセキュリティ問題に集中しましたが、本リリースは 2 点に重きを置いています。1 つ目はデータの可搬性と用量の可観測性の拡張で、S3 互換クラウド同期（WebDAV に並ぶ 2 つ目のクラウドバックアップバックエンド）、OpenCode セッション用量同期、公式サブスクリプション向けの残量統計テンプレートを追加しました。2 つ目は、Codex がサードパーティプロバイダーを Chat Completions ルーティングする際に露呈したエッジケースの継続的な改善で、ストリーム切断の判定、tools が空のときの `tool_choice`、カスタムツールのメタデータ、推論トークン統計、ファイル / 音声添付の変換などです。
...
ローカルプロキシは、Codex CLI が起動時にプローブする `GET /v1/models` に応答し、聚游助手 が管理する Codex モデルカタログを返すようになりました。あわせて古いカタログのガードを追加: live の `config.toml` を解析し、`model_catalog_json` が 聚游助手 所有のカタログファイルを指している場合のみ提供することで、前のプロバイダーが残したカタログを Codex に見せてしまうことを防ぎます（#3818）。
...
Chat Completions 上流が `finish_reason` も `[DONE]` もなくストリームを終了した場合、聚游助手 はこれを正常完了として扱わなくなりました: 本当に終了したときのみ正常に締め、部分的な出力があった場合は incomplete（`max_output_tokens`）レスポンスを、何も出力されなかった場合は失敗 `stream_truncated` イベントを発行します。遅れて届いた推論も、まだアクティブなストリーミングのツール呼び出しへバックフィルされます。
...
Codex 上流ゲートウェイが過大なリクエストボディを HTTP 413 で拒否したとき、プロキシはこれが 聚游助手 のローカル制限ではなくプロバイダーのサーバー側ボディサイズ制限であることを説明する専用メッセージを返し、実行可能な回復手順（`/compact` の実行、大きなログやインライン画像の削除、プロバイダーへの上限引き上げ依頼）を提示するようになりました。上流の生の HTML エラーページをそのまま返すことはなくなりました。
...
Codex は `config.toml` に絶対パスではなく相対ファイル名 `juyou-switcher-model-catalog.json` のみを書き込むようになりました（Codex CLI は設定ディレクトリから解決します）。これにより、絶対パスを変換できない WSL やシンボリックリンク環境でモデルカタログが壊れる問題を修正しました（#3614）。
...
Windows では 聚游助手 を終了すると、マウスを重ねるまで無効なトレイアイコンが残ることがありました。アプリは終了前にトレイアイコンを明示的に削除するようになり、プロセス終了とともにきれいに消えます（#3797）。

### Windows タスクバーアイコン

実行時に Windows AppUserModelID を明示的に設定し、インストーラーが生成するデスクトップとスタートメニューのショートカットに同じ ID と製品アイコンを書き込みます。これにより 聚游助手 がタスクバーで正しいアイコンを表示し、正しくグループ化されます（#3457）。
...
**Codex サードパーティプロバイダー Chat ルーティング**: 聚游助手 ローカルプロキシで Codex リクエストを変換し、サードパーティプロバイダーへ転送する場合、課金、コンプライアンス、データ保持に関する制約はプロバイダーごとに異なります。利用前に対象プロバイダーの利用規約を確認してください。

**Claude Desktop サードパーティプロバイダープロキシ切り替え**: 聚游助手 内蔵のプロキシゲートウェイで Claude Desktop のリクエストをサードパーティプロバイダーへ転送する場合も、対象プロバイダーの課金、コンプライアンス、データ保持に関する規約に従う必要があります。

上記機能を有効化したユーザーは、関連するリスクを自ら負うものとします。聚游助手 は、これらの機能の利用によって発生したアカウント制限、警告、サービス停止について責任を負いません。
...
- [#1351](https://github.com/yangp0419/juyou-switcher/pull/1351): S3 互換クラウドストレージ同期を追加、@keithyt06 に感謝。
- [#3215](https://github.com/yangp0419/juyou-switcher/pull/3215): OpenCode セッション用量同期を追加、@nothingness0db に感謝。
- [#2709](https://github.com/yangp0419/juyou-switcher/pull/2709): ZenMux Token Plan プロバイダーを追加、@Eter365 に感謝。
- [#3643](https://github.com/yangp0419/juyou-switcher/pull/3643): CherryIN プリセットプロバイダーを追加、@zhibisora に感謝。
- [#3818](https://github.com/yangp0419/juyou-switcher/pull/3818): Codex CLI 到達性確認用の `GET /v1/models` エンドポイントを追加、@CSberlin に感謝。
- [#3426](https://github.com/yangp0419/juyou-switcher/pull/3426): 用量ダッシュボードのヒーロー再設計、@allenxu09 に感謝。
- [#3640](https://github.com/yangp0419/juyou-switcher/pull/3640): tools が空のとき `tool_choice` を破棄、@Postroggy に感謝。
- [#3644](https://github.com/yangp0419/juyou-switcher/pull/3644): Chat ルーティングで Codex カスタムツールメタデータを保持、@LanternCX に感謝。
- [#3514](https://github.com/yangp0419/juyou-switcher/pull/3514): Chat→Responses で常に `reasoning_tokens` を含める、@yeeyzy に感謝。
- [#3689](https://github.com/yangp0419/juyou-switcher/pull/3689): live がすでにプロキシプレースホルダーのときバックアップ / 復元をスキップ、@YongmaoLuo に感謝。
- [#3016](https://github.com/yangp0419/juyou-switcher/pull/3016): localhost リッスンアドレスを正規化、@Alexlangl に感謝。
- [#3775](https://github.com/yangp0419/juyou-switcher/pull/3775): Anthropic `system` メッセージを正規化、@Dearli666 に感謝。
- [#3656](https://github.com/yangp0419/juyou-switcher/pull/3656): プロキシパネルのエラー表示を改善、@lzcndm に感謝。
- [#2647](https://github.com/yangp0419/juyou-switcher/pull/2647): 無限空白検出のしきい値を 20 → 500 へ引き上げ、@NiuBlibing に感謝。
- [#3702](https://github.com/yangp0419/juyou-switcher/pull/3702): 智譜の残量照会を構成済み base URL へルーティング、@YongmaoLuo に感謝。
- [#3518](https://github.com/yangp0419/juyou-switcher/pull/3518): MiniMax の新残量 API と既定価格に対応、@LaoYueHanNi に感謝。
- [#3524](https://github.com/yangp0419/juyou-switcher/pull/3524): 智譜 Coding Plan プリセットとバージョン付き endpoint のモデル探索を修正、@makoMakoGo に感謝。
- [#3614](https://github.com/yangp0419/juyou-switcher/pull/3614): モデルカタログを相対ファイル名に変更、@steponeerror に感謝。
- [#3797](https://github.com/yangp0419/juyou-switcher/pull/3797): Windows 終了後のトレイアイコン残留を修正、@iAJue に感謝。
- [#3457](https://github.com/yangp0419/juyou-switcher/pull/3457): Windows タスクバーアイコンを修正、@ZhangNanNan1018 に感謝。
- [#3430](https://github.com/yangp0419/juyou-switcher/pull/3430): Windows のパス区切りを正規化してサブディレクトリスキルの更新に対応、@Ninthless に感謝。
- [#3626](https://github.com/yangp0419/juyou-switcher/pull/3626): macOS の入力自動大文字化を無効化、@ZHLHZHU に感謝。
- [#3593](https://github.com/yangp0419/juyou-switcher/pull/3593): Codex VS Code セッションプレビューを修正、@xwil1 に感謝。
- [#3228](https://github.com/yangp0419/juyou-switcher/pull/3228): 中国語 UI の VS Code 表記を揃える、@Games55k に感謝。
- [#3411](https://github.com/yangp0419/juyou-switcher/pull/3411): 現行のアプリ対応を反映してユーザーマニュアルを刷新、@makoMakoGo に感謝。
- [#3772](https://github.com/yangp0419/juyou-switcher/pull/3772): README のリリースノートリンクとスポンサー表記を修正、@null-easy に感謝。
...
[Releases](https://github.com/yangp0419/juyou-switcher/releases/latest) から、お使いのシステムに対応するビルドをダウンロードしてください。
...
| ファイル                                 | 説明                                       |
| ---------------------------------------- | ------------------------------------------ |
| `juyou-switcher-v3.16.2-Windows.msi`          | **推奨** - 自動更新対応の MSI インストーラー |
| `juyou-switcher-v3.16.2-Windows-Portable.zip` | ポータブル版、展開してそのまま実行できます |

### macOS

| ファイル                         | 説明                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `juyou-switcher-v3.16.2-macOS.dmg`    | **推奨** - DMG インストーラー、Applications へドラッグ |
| `juyou-switcher-v3.16.2-macOS.zip`    | 展開して Applications へドラッグ、Universal Binary      |
| `juyou-switcher-v3.16.2-macOS.tar.gz` | Homebrew インストールと自動更新用                      |
...
| --------------------------------------- | ----------- | ------------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i juyou-switcher-*.deb` または `sudo apt install ./juyou-switcher-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i juyou-switcher-*.rpm` または `sudo dnf install ./juyou-switcher-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./juyou-switcher-*.rpm`                                   |
| Arch Linux / Manjaro                    | `.AppImage` | 実行権限を付与して直接起動、または AUR を使用                             |
| その他 / 不明                           | `.AppImage` | `chmod +x juyou-switcher-*.AppImage && ./juyou-switcher-*.AppImage`                 |
