...
- **Application-Level Window Controls**: Opt-in setting to render 聚游助手's own min/max/close buttons, materially improving Linux Wayland experience
- **Hermes in Unified Skills Management**: Skill install, enable, and filter now cover Hermes
- **Hermes / OpenClaw Config Directory Override**: Point 聚游助手 at a custom `~/.hermes/config.yaml` or `openclaw.json` location
...
聚游助手 now treats Hermes Agent as a first-class managed app alongside Claude / Codex / Gemini / OpenCode / OpenClaw.
...
- **User `providers:` Dict Read-Only Rendering**: User-authored providers in the YAML appear as read-only cards in 聚游助手; deep configuration delegates to the Hermes Web UI
...
- Opt-in setting to render 聚游助手's own minimize / toggle-maximize / close buttons instead of system decorations (#1119, thanks @git1677967754)
...
- New settings option to point 聚游助手 at a custom `openclaw.json` location (#1518, thanks @mrFranklin)

### Hermes Config Directory Override

- New settings option to point 聚游助手 at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch
...
- Deep YAML knobs are no longer duplicated in the 聚游助手 form — they are delegated to the Hermes Web UI via a direct launch action
...
- The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"juyou-switcher"`) across all call sites (#1978, thanks @lidaxian121)
...
- Round-tripping through 聚游助手 no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys
...
| File                                     | Description                                     |
| ---------------------------------------- | ----------------------------------------------- |
| `juyou-switcher-v3.14.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `juyou-switcher-v3.14.0-Windows-Portable.zip` | Portable, extract and run, no registry writes   |

### macOS

| File                             | Description                                              |
| -------------------------------- | -------------------------------------------------------- |
| `juyou-switcher-v3.14.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications  |
| `juyou-switcher-v3.14.0-macOS.zip`    | Extract and drag into Applications, Universal Binary     |
| `juyou-switcher-v3.14.0-macOS.tar.gz` | For Homebrew installation and auto-update                |
