...
> Codex stability patch: because some users did not want 聚游助手 to change how Codex config files are written, Codex App Enhancements now has a switch and is off by default. After enabling it, you can keep using Codex mobile remote control, official plugins, and other official-app features while using third-party APIs; this release also includes a series of stability fixes.
...
聚游助手 v3.16.1 is a Codex stability patch following v3.16.0. v3.16.0 promoted third-party Codex providers to first-class citizens through Chat Completions routing; this release focuses on several high-risk edges discovered in real use: official ChatGPT / Codex OAuth login state could be overwritten while switching third-party providers or during local routing takeover, the Codex model catalog could be cleared during live backfill, hot switching, takeover shutdown restore, or editing the active provider, and Codex `tool_search`, plugin / connector namespace tools, and custom tools were not fully restored back into Responses events on the Chat Completions upstream path.

This release also hardens local routing takeover ownership checks. Provider switching and takeover toggles now run serially per app. When deciding whether the live files are proxy-managed, 聚游助手 no longer relies only on stale `enabled` state or whether the proxy service is currently running; it also checks backups and proxy placeholders in the live files. This prevents ordinary live writes from overwriting proxy-managed config immediately after takeover is enabled, while the proxy is temporarily stopped, or during hot switching.
...
Added an optional setting for preserving official ChatGPT / Codex OAuth login state when switching to third-party Codex providers. When enabled, 聚游助手 stores third-party provider API keys in the provider-scoped `experimental_bearer_token` inside Codex `config.toml` instead of overwriting the official login cache in `auth.json`.
...
When hot switching Codex providers during local routing takeover, 聚游助手 refreshes the provider id, model, and display name in the live config so the Codex client menu follows the active provider. The base URL still stays pointed at the local proxy, preventing the real upstream endpoint from leaking back into the live file.
...
When Codex is under local routing takeover, live `auth.json` / `config.toml` are temporarily rewritten by the proxy. Editing the active provider from those live files could incorrectly show proxy placeholders or official OAuth login as provider config. The editor now explicitly explains that it is showing the provider config stored in the database, not the proxy-managed live files; even if the proxy service is temporarily stopped, 聚游助手 still treats the app as under takeover when the takeover state indicates so.
...
Provider switching also now always refreshes the generated Codex model catalog JSON ([#3360](https://github.com/yangp0419/juyou-switcher/pull/3360), thanks @Postroggy).
...
When Codex forwarding fails, 聚游助手 now returns JSON errors that include provider, model, endpoint, upstream HTTP status, stable `juyou_switcher_*` error codes, and normalized HTTP status. This makes it much clearer which provider, endpoint, and upstream error caused the failure.

### Codex Native Balance / Coding Plan Credential Lookup

Fixed native balance and Coding Plan queries using credentials from the wrong app. Each app now resolves its own provider credentials instead of carrying authentication assumptions from another app surface into the query flow ([#3355](https://github.com/yangp0419/juyou-switcher/pull/3355), thanks @SiskonEmilia).

### Codex CLI Discovery and Model Catalog Template Fallback

Fixed a too-narrow Codex CLI discovery path for third-party Codex model catalog projection. The backend now searches common Codex CLI install locations across platforms, and falls back to a built-in GPT-5.5 model catalog template if no template can be found ([#3382](https://github.com/yangp0419/juyou-switcher/pull/3382), thanks @chofuhoyu).

### Claude Desktop Official Provider Add Failure

Fixed an error when adding the Claude Desktop Official provider ([#3405](https://github.com/yangp0419/juyou-switcher/pull/3405), thanks @Eunknight).

### Kimi / Moonshot Tool-Thinking History Normalization

Added Kimi / Moonshot to the Anthropic-compatible tool-thinking history normalizer. Later turns can now correctly replay reasoning and tool-call context, avoiding failures caused by history messages that do not match upstream requirements ([#3377](https://github.com/yangp0419/juyou-switcher/pull/3377), thanks @Neon-Wang).
...
When local routing takeover is enabled, live `auth.json` / `config.toml` temporarily point to the 聚游助手 proxy. The provider editor therefore shows the provider config saved in the database. This is expected; after takeover is disabled, 聚游助手 restores live config from backups or the database source of truth.
...
- [#3360](https://github.com/yangp0419/juyou-switcher/pull/3360): always update Codex model catalog JSON when switching providers, thanks @Postroggy.
- [#3355](https://github.com/yangp0419/juyou-switcher/pull/3355): resolve native balance / Coding Plan credentials per app, thanks @SiskonEmilia.
- [#3405](https://github.com/yangp0419/juyou-switcher/pull/3405): fix Claude Desktop Official provider add failure, thanks @Eunknight.
- [#3382](https://github.com/yangp0419/juyou-switcher/pull/3382): Codex CLI multi-platform discovery and GPT-5.5 model template fallback, thanks @chofuhoyu.
- [#3377](https://github.com/yangp0419/juyou-switcher/pull/3377): Kimi / Moonshot tool-thinking history normalization, thanks @Neon-Wang.
...
| File                                     | Description                                     |
| ---------------------------------------- | ----------------------------------------------- |
| `juyou-switcher-v3.16.1-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `juyou-switcher-v3.16.1-Windows-Portable.zip` | Portable build, unzip and run                   |

### macOS

| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `juyou-switcher-v3.16.1-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `juyou-switcher-v3.16.1-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `juyou-switcher-v3.16.1-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `juyou-switcher-v3.16.1-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `juyou-switcher-v3.16.1-Linux-arm64.AppImage` / `.deb` / `.rpm`
