...
**[聚游助手 で Claude Desktop プロバイダーを一括設定・管理・切り替える](../user-manual/ja/2-providers/2.6-claude-desktop.md)**
...
聚游助手 は初めて **Claude Desktop** を一等管理対象パネルとして扱い、Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes と並列に位置づけます。

- Claude Desktop 専用パネルを追加し、聚游助手 内蔵プロキシゲートウェイを介してサードパーティプロバイダーを Claude Desktop に代理転送
...
ChatGPT Codex 系プロバイダーはハードコードされたモデル選択に依存しなくなり、聚游助手 が必要に応じて ChatGPT バックエンドから**ライブモデルリスト**を取得します。
...
Claude Desktop のプロバイダーを切り替えると 聚游助手 管理の 3P プロファイルが書き込まれます。**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 聚游助手 の Local Routing が動作し続けている必要があります。
...
### 聚游助手 ブランド公式サイトを github.com/yangp0419/juyou-switcher に統一

アプリ内および README 内のすべての「公式サイト」参照を、唯一の公式サイトとして github.com/yangp0419/juyou-switcher に統一しました; Release notes テンプレートにも github.com/yangp0419/juyou-switcher を反映。
...
Release notes テンプレート内に `github.com/yangp0419/juyou-switcher` を反映しました。

### ブランド公式サイト

各 README およびアプリ内参照で `github.com/yangp0419/juyou-switcher` を唯一の公式サイトとしてドキュメント化しました。
...
これら 20 個のプリセットは以前はデフォルトでプロキシ経由でルーティングされていましたが、現在はデフォルトで直接接続です。アップグレード前にこのうちのいずれかを使用しており、かつプロキシルーティングの接続性に依存していた場合（例: プロキシに特殊な補正器や変換層がある場合）、接続性を検証してください; 必要に応じて、聚游助手 パネル内で手動でプロキシモードに戻すことができます。

### Claude Desktop の操作制約

Claude Desktop プロバイダーの切り替えには、**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 聚游助手 の Local Routing が動作し続けている必要があります — 聚游助手 を終了させたり Local Routing を停止させたりすると、プロキシモードの Claude Desktop プロバイダーへの接続が切断されます。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `juyou-switcher-v3.15.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `juyou-switcher-v3.15.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `juyou-switcher-v3.15.0-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ |
| `juyou-switcher-v3.15.0-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary     |
| `juyou-switcher-v3.15.0-macOS.tar.gz` | Homebrew インストールと自動更新用                      |
