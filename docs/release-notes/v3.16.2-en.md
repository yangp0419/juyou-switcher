...
>
> 聚游助手 is a **fully free and open-source** desktop app, and we **do not charge users any fees**. Please only obtain the software through the official channels listed below:
>
> | Channel            | Only Official                                                                  |
> | ------------------ | ------------------------------------------------------------------------------ |
> | Website            | **[github.com/yangp0419/juyou-switcher](https://github.com/yangp0419/juyou-switcher)**                                         |
> | Source             | **[github.com/yangp0419/juyou-switcher](https://github.com/yangp0419/juyou-switcher)** |
> | Downloads          | **[GitHub Releases](https://github.com/yangp0419/juyou-switcher/releases)**        |
> | Author             | **[@yangp0419](https://github.com/yangp0419)**                               |
> | Report an Imposter | **[GitHub Issues](https://github.com/yangp0419/juyou-switcher/issues)**            |
>
> **Any "聚游助手" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues.

---

## Overview

聚游助手 v3.16.2 is a maintenance update following v3.16.1. After the previous release focused on the security of Codex official authentication and local routing takeover, this release concentrates on two things. First, broadening data portability and usage observability — adding S3-compatible cloud sync (a second cloud-backup backend alongside WebDAV), OpenCode session usage sync, and a quota-statistics template for official subscriptions. Second, continuing to polish the edges exposed when Codex routes third-party providers through Chat Completions — stream-truncation detection, `tool_choice` when tools is empty, custom-tool metadata, reasoning-token statistics, file / audio attachment conversion, and more.
...
The local proxy now responds to `GET /v1/models`, which Codex CLI probes at startup, returning the 聚游助手-managed Codex model catalog. A stale-catalog guard was added: it parses the live `config.toml` and only serves the catalog when `model_catalog_json` still points at the 聚游助手-owned catalog file, avoiding exposing a previous provider's leftover catalog to Codex (#3818).
...
When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, 聚游助手 no longer treats it as a normal completion: it finalizes normally only when the stream truly ended; emits an incomplete (`max_output_tokens`) response when partial output was produced; and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also backfilled onto still-active streaming tool calls.
...
Codex now writes only the relative filename `juyou-switcher-model-catalog.json` to `config.toml` instead of an absolute path (Codex CLI resolves it from the config directory), fixing the model catalog breaking on WSL and symlinked setups where the absolute path could not be translated (#3614).
...
On Windows, quitting 聚游助手 could leave a dead tray icon behind until the mouse passed over it. The app now explicitly removes the tray icon before exiting, so it disappears cleanly when the process ends (#3797).

### Windows Taskbar Icon

Sets an explicit Windows AppUserModelID at runtime and writes the same ID and product icon onto the installer's desktop and start-menu shortcuts, so 聚游助手 shows the correct icon and groups properly in the taskbar (#3457).
...
**Codex third-party provider Chat routing**: when 聚游助手 local proxy converts and forwards Codex requests to third-party providers, each provider may have different requirements for billing, compliance, and data retention. Read the target provider's terms before use.

**Claude Desktop third-party provider proxy switching**: when 聚游助手's built-in proxy gateway forwards Claude Desktop requests to third-party providers, you must also follow the target provider's billing, compliance, and data-retention terms.

By enabling these features, users accept the related risks. 聚游助手 is not responsible for account restrictions, warnings, or service suspensions caused by using these features.
...
- [#1351](https://github.com/yangp0419/juyou-switcher/pull/1351): add S3-compatible cloud storage sync, thanks @keithyt06.
- [#3215](https://github.com/yangp0419/juyou-switcher/pull/3215): add OpenCode session usage sync, thanks @nothingness0db.
- [#2709](https://github.com/yangp0419/juyou-switcher/pull/2709): add the ZenMux Token Plan provider, thanks @Eter365.
- [#3643](https://github.com/yangp0419/juyou-switcher/pull/3643): add the CherryIN preset provider, thanks @zhibisora.
- [#3818](https://github.com/yangp0419/juyou-switcher/pull/3818): add the Codex CLI reachability `GET /v1/models` endpoint, thanks @CSberlin.
- [#3426](https://github.com/yangp0419/juyou-switcher/pull/3426): Usage Dashboard hero redesign, thanks @allenxu09.
- [#3640](https://github.com/yangp0419/juyou-switcher/pull/3640): drop `tool_choice` when tools is empty, thanks @Postroggy.
- [#3644](https://github.com/yangp0419/juyou-switcher/pull/3644): preserve Codex custom tool metadata in chat routing, thanks @LanternCX.
- [#3514](https://github.com/yangp0419/juyou-switcher/pull/3514): always include `reasoning_tokens` in Chat→Responses, thanks @yeeyzy.
- [#3689](https://github.com/yangp0419/juyou-switcher/pull/3689): skip backup / restore when live is already a proxy placeholder, thanks @YongmaoLuo.
- [#3016](https://github.com/yangp0419/juyou-switcher/pull/3016): normalize the localhost listen address, thanks @Alexlangl.
- [#3775](https://github.com/yangp0419/juyou-switcher/pull/3775): normalize Anthropic `system` messages, thanks @Dearli666.
- [#3656](https://github.com/yangp0419/juyou-switcher/pull/3656): improve error message display in the proxy panel, thanks @lzcndm.
- [#2647](https://github.com/yangp0419/juyou-switcher/pull/2647): raise the infinite-whitespace threshold 20 → 500, thanks @NiuBlibing.
- [#3702](https://github.com/yangp0419/juyou-switcher/pull/3702): route the Zhipu quota query to the configured base URL, thanks @YongmaoLuo.
- [#3518](https://github.com/yangp0419/juyou-switcher/pull/3518): adapt to the MiniMax new balance API and default pricing, thanks @LaoYueHanNi.
- [#3524](https://github.com/yangp0419/juyou-switcher/pull/3524): fix the Zhipu Coding Plan presets and model probing for versioned endpoints, thanks @makoMakoGo.
- [#3614](https://github.com/yangp0419/juyou-switcher/pull/3614): use a relative filename for the model catalog, thanks @steponeerror.
- [#3797](https://github.com/yangp0419/juyou-switcher/pull/3797): fix the Windows tray icon residue after exit, thanks @iAJue.
- [#3457](https://github.com/yangp0419/juyou-switcher/pull/3457): fix the Windows taskbar icon, thanks @ZhangNanNan1018.
- [#3430](https://github.com/yangp0419/juyou-switcher/pull/3430): normalize Windows path separators to match subdirectory skill updates, thanks @Ninthless.
- [#3626](https://github.com/yangp0419/juyou-switcher/pull/3626): disable macOS input auto-capitalization, thanks @ZHLHZHU.
- [#3593](https://github.com/yangp0419/juyou-switcher/pull/3593): fix Codex VS Code session previews, thanks @xwil1.
- [#3228](https://github.com/yangp0419/juyou-switcher/pull/3228): align the VS Code wording in the Chinese UI, thanks @Games55k.
- [#3411](https://github.com/yangp0419/juyou-switcher/pull/3411): refresh the user manual to reflect current app support, thanks @makoMakoGo.
- [#3772](https://github.com/yangp0419/juyou-switcher/pull/3772): fix README release-note links and sponsor markup, thanks @null-easy.
...
Visit [Releases](https://github.com/yangp0419/juyou-switcher/releases/latest) and download the build for your system.
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `juyou-switcher-v3.16.2-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `juyou-switcher-v3.16.2-Windows-Portable.zip` | Portable build, unzip and run                    |

### macOS

| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `juyou-switcher-v3.16.2-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `juyou-switcher-v3.16.2-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `juyou-switcher-v3.16.2-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
| --------------------------------------- | ------------------ | --------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`             | `sudo dpkg -i juyou-switcher-*.deb` or `sudo apt install ./juyou-switcher-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`             | `sudo rpm -i juyou-switcher-*.rpm` or `sudo dnf install ./juyou-switcher-*.rpm`  |
...
| Other distributions / unsure            | `.AppImage`        | `chmod +x juyou-switcher-*.AppImage && ./juyou-switcher-*.AppImage`             |
