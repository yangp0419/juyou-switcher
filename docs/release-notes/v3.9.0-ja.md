...
聚游助手 v3.9.0 は v3.9 ベータ（`3.9.0-1`、`3.9.0-2`、`3.9.0-3`）の安定版です。
...
- SQL インポートは 聚游助手 がエクスポートしたバックアップのみに制限されます（安全性のため）。
- プロキシのテイクオーバーは CLI の live 設定を変更します。聚游助手 はリダイレクト前に live 設定をバックアップします。元に戻す場合はテイクオーバー無効化/プロキシ停止を行い、必要に応じてバックアップから復元してください。
...
[Releases](https://github.com/yangp0419/juyou-switcher/releases/latest) から該当するバージョンをダウンロードしてください。
...
| ファイル                                | 説明                                         |
| --------------------------------------- | -------------------------------------------- |
| `juyou-switcher-v3.9.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応  |
| `juyou-switcher-v3.9.0-Windows-Portable.zip` | ポータブル版、インストール不要               |

### macOS

| ファイル                        | 説明                                                              |
| ------------------------------- | ----------------------------------------------------------------- |
| `juyou-switcher-v3.9.0-macOS.zip`    | **推奨** - 解凍して Applications へドラッグ、Universal Binary     |
| `juyou-switcher-v3.9.0-macOS.tar.gz` | Homebrew インストールおよび自動更新用                             |
...
| サンドボックスで実行したい場合          | `.flatpak`  | `flatpak install juyou-switcher-*.flatpak`                                          |
