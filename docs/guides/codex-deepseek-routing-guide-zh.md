# 在 Codex 中用 DeepSeek 这类 Chat 格式 API：聚游助手 本地路由攻略

> 适用版本：聚游助手 3.16.0 及附近版本。本文根据仓库内文档与代码整理，并用 DeepSeek 作为 OpenAI Chat Completions 兼容接口的示例。截图来自当前前端界面，使用去敏示例数据生成，避免泄露真实 API Key 或账户余额。
...
聚游助手 的做法是让 Codex 始终连本机路由，仍以 Responses API 发送请求；路由在内部识别当前供应商是否是 Chat 格式，再把请求改写成 Chat Completions 发给上游，最后把 Chat 响应转换回 Responses 形态返回给 Codex。
...
DeepSeek 官方文档目前写明 OpenAI 兼容 base URL 是 `https://api.deepseek.com`（其他供应商常见的是带 `/v1` 后缀的 base URL），Chat API 路径是 `/chat/completions`；聚游助手 的 DeepSeek 预设已经按这些信息配好，请优先使用预设，不需要手动拼接口路径。

## 第一步：添加 Codex 供应商

打开 聚游助手，切到顶部的 `Codex` 标签，点击右上角的加号添加供应商。
...
接管后，聚游助手 会把 Codex 的 live 配置指向本机路由，并用占位符管理认证。真实 DeepSeek Key 仍保存在 聚游助手 的 Provider 配置里，由本地路由在转发时注入，不需要你把 Key 暴露给 Codex live 配置。

## 第三步：切换供应商并重启 Codex

回到 Codex 供应商列表，点击 DeepSeek 供应商的 `启用`。如果看到 `需要路由` 标记，说明这个供应商必须在路由运行时使用；没有启动路由时，聚游助手 会弹出“需要路由服务才能正常使用”的提示。
...
DeepSeek、Kimi、MiniMax、SiliconFlow 等常见 Chat 格式供应商在 聚游助手 里已有预设，优先用预设即可。只有预设里没有的供应商，才需要选择自定义配置；这时按对方文档填 API Key、base URL 和模型，并把 `API 格式` 选为 `OpenAI Chat Completions (需开启路由)`。

如果上游直接支持 OpenAI Responses API，就不需要打开 `需要本地路由映射`；这时 聚游助手 可以按 Responses 直连，不做 Chat 转换。
...
保存供应商后重启 Codex。聚游助手 会生成 `juyou-switcher-model-catalog.json` 并把路径写入 `model_catalog_json`，但正在运行的 Codex 进程不一定会热加载模型目录。
...
不建议。聚游助手 会在本地路由接管模式下阻止切到官方供应商，因为用代理访问官方 API 可能带来账号风险。路由主要用于第三方、聚合或协议转换场景。

## 参考链接

- [聚游助手 用户手册：添加供应商](../user-manual/zh/2-providers/2.1-add.md)
- [聚游助手 用户手册：代理服务](../user-manual/zh/4-proxy/4.1-service.md)
- [聚游助手 用户手册：应用路由](../user-manual/zh/4-proxy/4.2-routing.md)
