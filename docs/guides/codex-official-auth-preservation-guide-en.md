# Keep Codex Remote Control and Official Plugins While Using Third-Party APIs: 聚游助手 Setup Guide

> Applies to 聚游助手 v3.16.1 and later. This guide is based on the current code, user manual, and v3.16.1 release notes. Screenshots use de-identified sample data and do not include real Access Tokens or API keys.
...
1. In the 聚游助手 Codex panel, switch to `OpenAI Official`.
2. Start Codex and log in once with an official ChatGPT / Codex account. A Free subscription is enough.
3. Return to 聚游助手 and enable `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers`.
...
Open 聚游助手 and switch to the top-level `Codex` tab. First select the `OpenAI Official` provider, or add it from the preset providers if it is missing, and make it the current provider.
...
If it only supports OpenAI Chat Completions, which is common for DeepSeek / Kimi / MiniMax paths, local routing must be enabled so 聚游助手 can convert Codex Responses requests into Chat Completions requests.
...
After takeover, Codex's live `config.toml` temporarily points to the 聚游助手 local route. The real third-party API key remains in the 聚游助手 provider configuration, and is projected into the `experimental_bearer_token` in `config.toml` when providers are switched.
...
- In 聚游助手, the current Codex provider is the third-party provider.
...
After `Keep official login when switching third-party providers` is enabled, 聚游助手 takes the third-party provider API key from the provider configuration and writes it under the current provider in `config.toml`:
...
If the provider uses the Chat Completions protocol, 聚游助手 local routing adds another conversion layer:
...
That does not mean model requests are still going to official OpenAI. Actual traffic is determined by the current Codex provider in 聚游助手, `config.toml`, and local routing logs.
...
Codex reads the model catalog at startup. Even if 聚游助手 has generated a new model catalog, a running Codex process may not hot-load it, so restart Codex after editing model mappings.
...
This is expected. Official account information comes from `auth.json`; the actual model provider comes from `config.toml` and the current provider in 聚游助手.

**Is a Free subscription really enough?**

Yes. The official account is mainly used to obtain and preserve the official login state required by Codex App. Third-party model requests use the third-party API key configured in 聚游助手.

**What should I do if official plugins or mobile remote control still do not work?**

Switch back to `OpenAI Official`, restart Codex, and complete official login once. Then confirm `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers` is enabled in 聚游助手 before switching back to the third-party provider.
...
Not recommended. 聚游助手 tries to prevent switching to official providers while local routing takeover is active, because accessing official APIs through a proxy may create account risk. Use official login only to preserve `auth.json`, and route model traffic to third-party providers.
...
- [聚游助手 v3.16.1 Release Note](../release-notes/v3.16.1-en.md)
