# 使用第三方 API 时保留 Codex 远程操作和官方插件：聚游助手 配置攻略

> 适用版本：聚游助手 v3.16.1 及以上。本文根据当前代码、用户手册和 v3.16.1 Release Note 整理，截图使用去敏示例数据，不包含真实 Access Token 或 API Key。
...
v3.16.1 新增的 **Codex 应用增强**开关就是为了解决这个矛盾：让官方 Access Token 继续留在 `auth.json`，而第三方供应商信息写入 `config.toml`。这样 Codex App 仍然认为你登录的是官方账号，但实际模型请求会走 聚游助手 当前选中的第三方供应商。
...
1. 在 聚游助手 的 Codex 面板切换到 `OpenAI Official`。
2. 启动 Codex，并用官方 ChatGPT / Codex 账号登录一次，Free 订阅也可以。
3. 回到 聚游助手，打开 `设置 → 通用 → Codex 应用增强 → 切换第三方时保留官方登录`。
...
打开 聚游助手，切到顶部的 `Codex` 标签页。先选择 `OpenAI Official` 供应商（如果没有的话，就在预设供应商当中添加一个），并把它设为当前供应商。
...
如果它只支持 OpenAI Chat Completions，例如常见的 DeepSeek / Kimi / MiniMax 路径，就必须启用本地路由，让 聚游助手 把 Codex 的 Responses 请求转换成 Chat Completions 请求。
...
接管后，Codex 的 live `config.toml` 会临时指向 聚游助手 本地路由。真实第三方 API Key 仍然存储在 聚游助手 的供应商配置中，切换供应商时再投影到 `config.toml` 的 `experimental_bearer_token`。
...
- 在 聚游助手 里，当前 Codex 供应商显示为第三方供应商。
...
开启 `切换第三方时保留官方登录` 后，聚游助手 的切换逻辑会把第三方供应商 API Key 从供应商配置中取出，写到 `config.toml` 的当前 provider 下：
...
如果供应商是 Chat Completions 协议，聚游助手 本地路由会再做一层转换：
...
但这不代表模型请求还在走官方 OpenAI。实际流量以 聚游助手 当前 Codex 供应商、`config.toml` 和本地路由日志为准。
...
Codex 的模型目录是启动时读取的。即使 聚游助手 已经生成了新的模型目录，正在运行的 Codex 也不一定会热加载，所以修改模型映射后请重启 Codex。
...
这是预期行为。官方账号信息来自 `auth.json`，模型请求的实际供应商来自 `config.toml` 和 聚游助手 当前供应商。

**Free 订阅真的可以吗？**

可以。这里的官方账号主要用于获取并保留 Codex App 需要的官方登录态。第三方模型请求使用的是你在 聚游助手 里配置的第三方 API Key。

**开启后官方插件或手机远程操作还是不可用怎么办？**

先切回 `OpenAI Official`，重新启动 Codex 并完成一次官方登录；然后确认 聚游助手 的 `设置 → 通用 → Codex 应用增强 → 切换第三方时保留官方登录` 已开启，再切回第三方供应商。
...
不建议。聚游助手 会尽量阻止在本地路由接管模式下切到官方供应商，因为用代理访问官方 API 可能带来账号风险。建议官方登录只用于保留 `auth.json`，模型流量则切到第三方供应商。
...
- [聚游助手 v3.16.1 Release Note](../release-notes/v3.16.1-zh.md)
