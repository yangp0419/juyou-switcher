...
export function useProxyStatus() {
  ...
  // 强制恢复指定应用的 Live 接管配置
  const restoreTakeoverForAppMutation = useMutation({
    mutationFn: (appType: string) =>
      invoke("restore_proxy_takeover_for_app", { appType }),
    onSuccess: (_data, appType) => {
      const appLabel =
        appType === "claude"
          ? "Claude"
          : appType === "codex"
      ...
      toast.success(`已恢复 ${appLabel} 接管配置`, { closeButton: true });
      queryClient.invalidateQueries({ queryKey: ["proxyStatus"] });
      queryClient.invalidateQueries({ queryKey: ["proxyTakeoverStatus"] });
      queryClient.removeQueries({ queryKey: ["providerHealth"] });
      queryClient.removeQueries({ queryKey: ["circuitBreakerStats"] });
    ...
    onError: (error: Error) => {
      const detail =
        extractErrorMessage(error) ||
        t("common.unknown", { defaultValue: "未知错误" });
      toast.error(`恢复失败: ${detail}`, { closeButton: true });
    },
  });
  ...
  return {
    ...
    restoreTakeoverForApp: restoreTakeoverForAppMutation.mutateAsync,
    ...
    isRestoringTakeover: restoreTakeoverForAppMutation.isPending,
    isPending:
      ...
      restoreTakeoverForAppMutation.isPending,
  };
}
