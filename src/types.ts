...
export interface Provider {
  ...
  // 备注信息
  notes?: string;
  // 可选：供应商元数据（仅存于 ~/.juyou-switcher/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// Skill 存储位置
export type SkillStorageLocation = "juyou_switcher" | "unified";
...
// 存储在本地 ~/.juyou-switcher/settings.json，不随数据库同步
export interface Settings {
  ...
  // Skill 存储位置：juyou_switcher（默认）或 unified（~/.agents/skills/）
