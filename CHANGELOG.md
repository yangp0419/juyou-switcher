...
All notable changes to 聚游助手 will be documented in this file.
...
- **Codex CLI Models Endpoint**: The local proxy now answers `GET /v1/models`, which Codex CLI probes at startup, returning the juyou-switcher-managed Codex model catalog. A stale-catalog guard parses the live `config.toml` and only serves the catalog when `model_catalog_json` still references the juyou-switcher-owned file, so a leftover catalog from a previous provider is not advertised (#3818).
...
- **Codex Chat Truncated Stream Detection**: When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, 聚游助手 no longer reports it as a normal completion — it finalizes normally only when the stream truly finished, emits an incomplete (`max_output_tokens`) response when partial output was produced, and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also attached to still-active streamed tool calls.
...
- **Codex Model Catalog Path Portability**: Codex now writes only the relative filename `juyou-switcher-model-catalog.json` to `config.toml` instead of an absolute path (Codex CLI resolves it from the config directory), fixing the model catalog breaking on WSL and symlinked setups where the absolute path could not be translated (#3614).
...
- **Windows Tray Icon Residue on Exit**: Quitting 聚游助手 on Windows could leave a dead tray icon until hovered; the app now removes the tray icon before exiting so it disappears cleanly (#3797).
- **Windows Taskbar Icon**: Set an explicit Windows AppUserModelID at runtime and stamped the installer's desktop and start-menu shortcuts with the same ID and product icon, so 聚游助手 shows the correct icon and groups properly in the taskbar (#3457).
...
- **Codex Proxy Error Diagnostics**: Codex forwarding failures now return richer JSON errors with provider, model, endpoint, upstream status, stable `juyou_switcher_*` codes, and HTTP statuses aligned with the canonical `ProxyError` response mapping.
...
- **Codex Chat Completions routing**: Codex providers can now be served by OpenAI-compatible Chat Completions upstreams. 聚游助手 converts Codex Responses requests into Chat Completions, rebuilds JSON and SSE responses back into Responses shape, preserves reasoning / `<think>` / tool-call state, normalizes error envelopes, and probes Chat-format providers correctly in Stream Check.
...
- **Codex Chat Completions Routing**: Codex providers can now be served by upstreams that only speak the OpenAI Chat Completions API. 聚游助手's local proxy converts Codex's outgoing Responses requests into Chat Completions and rebuilds the Chat response (both JSON and SSE) back into Responses shape, preserving `reasoning_content`, inline `<think>` blocks, streamed reasoning summaries, tool calls, and `previous_response_id` follow-ups. A bounded Codex Chat history cache restores tool calls before their tool outputs.
...
- **Codex Model Mapping Table**: Codex provider forms now expose a model catalog (model + display name + context window per row) that is the single source of truth for the upstream model list, projected to `~/.codex/juyou-switcher-model-catalog.json`.
...
- **Homebrew Official Cask Installation**: Installation simplified to `brew install --cask juyou-switcher` now that 聚游助手 is in the official Homebrew repository; the personal-tap requirement was removed from all READMEs.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through 聚游助手's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes 聚游助手's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require 聚游助手 Local Routing to stay running.
...
- **聚游助手 Brand Surface Unified to github.com/yangp0419/juyou-switcher**: All in-app and README references now point at github.com/yangp0419/juyou-switcher as the sole official website; the release notes template also surfaces github.com/yangp0419/juyou-switcher.
...
- **Release Notes Template**: Surfaces `github.com/yangp0419/juyou-switcher` in the release notes template.
- **Brand Surface**: Documented `github.com/yangp0419/juyou-switcher` as the sole official website across READMEs and in-app references.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. 聚游助手 now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; 聚游助手 aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from 聚游助手, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render 聚游助手's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point 聚游助手 at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point 聚游助手 at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the 聚游助手 form.
...
- **Tray Menu ID Constant**: The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"juyou-switcher"`) across all call sites.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through 聚游助手 no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps 聚游助手 running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between 聚游助手 and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **Skill Auto-Backup**: Skill files are automatically backed up to `~/.juyou-switcher/skill-backups/` before uninstall, with metadata preserved in `meta.json`; old backups pruned to keep at most 20
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in 聚游助手, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Crash Logging** - Panic hook captures crash info to `~/.juyou-switcher/crash.log` with full stack traces (#562)
...
- **Windows Title** - Display "聚游助手" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to 聚游助手 exported backups only; refresh providers immediately after import
...
- **ArchLinux installation support** (#259) - Added AUR installation via `paru -S juyou-switcher-bin`
...
- **Deep link support** - Import Gemini providers via `juyouswitcher://` protocol
...
#### Deep Link Protocol (juyouswitcher://)

- **Protocol registration** - `juyouswitcher://` URL scheme for one-click imports
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `juyou-switcher-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `juyou-switcher-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `juyou-switcher-v{version}-Linux.AppImage` / `.deb`
...
#### Backup on v1→v2 Migration (juyou-switcher internal config)

- When the app detects an old v1 config structure at `~/.juyou-switcher/config.json`, it now creates a timestamped backup before writing the new v2 structure.
- Backup location: `~/.juyou-switcher/config.v1.backup.<timestamp>.json`
- This only concerns juyou-switcher's own metadata file; your actual provider files under `~/.claude/` and `~/.codex/` are untouched.
